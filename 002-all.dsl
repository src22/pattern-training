...
an immutable object with a builder
that stores constructor arguments in fields.

#13 Context Variable
Use a variable
to hold context
required during a parse.
...
by breaking it up into chunks
(usually lines)
and then parsing each chunk.
...
Embed interpreter actions
into the grammar,
so that executing the parser
causes the text
to be directly interpreted
to produce the response.

#25 Embedded Translation
Embed output production code
into the parser,
...
#54 Embedment Helper
An object that minimizes code
in a templating system
by providing all needed functions
to that templating mechanism.
...
#27 Foreign Code
Embed some foreign code
into an external DSL
to provide more elaborate behavior
...
before language processing
using Templated Generation.
...
so that there's no explicit representation
of the Semantic Model.

#55 Model-Aware Generation
Generate code with
an explicit simulacrum
of the semantic model of the DSL,
so that the generated code
has generic-specific separation.
...
that can recursively contain
the same form of expression
(for example, arithmetic and Boolean expressions).
...
#16 Notification
Collects errors and other messages
...
#22 Parser Combinator
Create a top-down parser
by a composition of
parser objects.

#23 Parser Generator
Build a parser driven by
a grammar file
as a DSL.
...
Create a top-down parser
using control flow
for grammar operators
and recursive functions
for nonterminal recognizers.

#20 Regex Table Lexer
...
Translate source text
by defining a grammar
and using that grammar
to structure translation.

#53 Templated Generation
Generate output
by handwriting an output file
and placing template callouts
...
that navigates the input model
and produces output.

#24 Tree Construction
The parser creates and returns
a syntax tree representation
of the source text
that is manipulated later
by tree-walking code.
